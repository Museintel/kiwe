...
23. Later batches can safely apply the staged/proven/authorized/gated/previewed/confirmed/fresh/rollback-ready/target-locked/rollback-captured/render-inspected/shelled/save-approved/executor-planned/adapter-planned candidate through Bricks 2.4 abilities or Bricks import workflows only after post-apply verification, rollback proof, Kiwe audit, and browser smoke tests are wired. Until that future proof exists, the CLI and WordPress admin apply-plan views/downloads/stages/proofs/authorizations/gates/previews/confirmations/revalidations/rollback-checkpoints/target-locks/rollback-captures/render-inspections/adapter-shells/save-approvals/executor-skeletons/adapter-plans are inspection, approval, interface, and restore-prep contracts only.
