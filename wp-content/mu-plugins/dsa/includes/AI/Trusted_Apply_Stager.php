...
final class Trusted_Apply_Stager {
	...
	public function attach_rollback_capture( string $id, array $capture ): array {
		$records = $this->records();
		$updated = [];
		$matched = false;
		foreach ( $records as $record ) {
			...
			if ( $id === (string) ( $record['id'] ?? '' ) ) {
				$record['rollbackCapture']    = $capture;
				$record['rollbackCapturedAt'] = (string) ( $capture['createdAt'] ?? gmdate( 'c' ) );
				$matched                      = true;
			}
			$updated[] = $record;
		}
	...
	public function attach_rendered_target_inspection( string $id, array $inspection ): array {
		$records = $this->records();
		$updated = [];
		$matched = false;
		foreach ( $records as $record ) {
			...
			if ( $id === (string) ( $record['id'] ?? '' ) ) {
				$record['renderedTargetInspection'] = $inspection;
				$record['renderedInspectedAt']      = (string) ( $inspection['createdAt'] ?? gmdate( 'c' ) );
				$matched                            = true;
			}
			$updated[] = $record;
		}
	...
	public function attach_minimal_adapter_shell( string $id, array $shell ): array {
		$records = $this->records();
		$updated = [];
		$matched = false;
		foreach ( $records as $record ) {
			...
			if ( $id === (string) ( $record['id'] ?? '' ) ) {
				$record['minimalAdapterShell'] = $shell;
				$record['adapterShelledAt']    = (string) ( $shell['createdAt'] ?? gmdate( 'c' ) );
				$matched                       = true;
			}
			$updated[] = $record;
		}
	...
	public function attach_final_save_approval( string $id, array $approval ): array {
		$records = $this->records();
		$updated = [];
		$matched = false;
		foreach ( $records as $record ) {
			...
			if ( $id === (string) ( $record['id'] ?? '' ) ) {
				$record['finalSaveApproval'] = $approval;
				$record['saveApprovedAt']    = (string) ( $approval['createdAt'] ?? gmdate( 'c' ) );
				$matched                     = true;
			}
			$updated[] = $record;
		}
	...
	public function attach_controlled_executor( string $id, array $executor ): array {
		$records = $this->records();
		$updated = [];
		$matched = false;
		foreach ( $records as $record ) {
			...
			if ( $id === (string) ( $record['id'] ?? '' ) ) {
				$record['controlledExecutor'] = $executor;
				$record['executorBuiltAt']    = (string) ( $executor['createdAt'] ?? gmdate( 'c' ) );
				$matched                      = true;
			}
			$updated[] = $record;
		}
	...
	public function attach_bricks_controlled_adapter( string $id, array $adapter ): array {
		$records = $this->records();
		$updated = [];
		$matched = false;
		foreach ( $records as $record ) {
			...
			if ( $id === (string) ( $record['id'] ?? '' ) ) {
				$record['bricksControlledAdapter'] = $adapter;
				$record['bricksAdapterPreparedAt'] = (string) ( $adapter['createdAt'] ?? gmdate( 'c' ) );
				$matched = true;
			}
			$updated[] = $record;
		}
	...
	public function attach_post_apply_verification( string $id, array $verification ): array {
		$records = $this->records();
		$updated = [];
		$matched = false;
		foreach ( $records as $record ) {
			...
			if ( $id === (string) ( $record['id'] ?? '' ) ) {
				$record['postApplyVerification'] = $verification;
				$record['postApplyVerifiedAt']   = (string) ( $verification['createdAt'] ?? gmdate( 'c' ) );
				$matched                         = true;
			}
			$updated[] = $record;
		}
