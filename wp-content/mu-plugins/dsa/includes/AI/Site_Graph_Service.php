...
final class Site_Graph_Service {
	...
	public function calibration_profile(): array {
		...
		return [
			...
			'bricks'      => [
				...
				'surfaceStyleControls' => class_exists( '\\Bricks\\Setup' ) ? \DSA\Bricks\Surface_Style_Controls::capabilities( ! empty( $this->settings->all()['bricks']['add_to_cart_enhancer_enabled'] ) ) : [],
			],
		];
	}
	...
	private function connectors_summary(): array {
		return [
			...
			'accessibility' => [
				...
				'validationSchema' => 'kiwe.accessibility-validation.v1',
				'readOnly'    => true,
				'mutatesContent' => false,
	...
	private function kiwe_dynamic_tags(): array {
		return [
			...
			'{kiwe_business_story}',
			'{kiwe_business_mission}',
			'{kiwe_business_vision}',
			'{kiwe_business_values}',
			'{kiwe_business_usp}',
			'{kiwe_founder_name}',
			'{kiwe_founder_title}',
			'{kiwe_founder_bio}',
			'{kiwe_founder_image}',
			'{kiwe_founder_linkedin_url}',
			'{kiwe_team_enabled}',
			'{kiwe_team_member_id}',
			'{kiwe_team_name}',
			'{kiwe_team_title}',
