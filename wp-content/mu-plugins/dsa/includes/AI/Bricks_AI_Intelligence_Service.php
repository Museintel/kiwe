...
final class Bricks_AI_Intelligence_Service {
	...
	public function planning_packet( array $args = [] ): array {
		...
		return [
			...
			'plan'      => [
				...
				'conversion'        => [
					'rule' => 'When the human asks /convert /bricks, emit one token-pure native Bricks My Templates upload JSON at bricks-template/[page]-template-upload.json by default. Preserve approved layout and source semantics while mapping live data intent to Bricks-native elements/settings. Element-native settings are the render/edit owner for full-page template uploads; imported global_classes are semantic/name-only and must not duplicate paint, layout, radius, spacing, shadows, or typography. Native settings must consume Kiwe/Seam token variables, declared project variables, or real tokenized clamp() values for spacing, sizing, radius, type, shadow, transform offsets, and responsive layout; do not emit bare design lengths such as 28px, 2.35rem, or 390px, and do not use no-op clamps such as clamp(22px, 22px, 22px). For human My Templates upload, every CSS variable used in native settings must include a safe fallback, e.g. var(--kiwe-color-surface, #fff8ef), because Bricks cannot be trusted to hydrate top-level globalVariables from the same JSON on every target site. Store native Bricks global variable names without leading -- because Bricks emits the prefix itself. Use Bricks source-backed sizing controls (_widthMax/_widthMin/_heightMax/_heightMin), not _maxWidth/_minWidth/_maxHeight/_minHeight; store border radius as _border.radius.top/right/bottom/left, not topLeft/topRight/bottomRight/bottomLeft; and do not place var(...) font stacks in _typography.font-family because Bricks quotes font-family output. Semantic Bricks heading elements (`name: heading` with `tag: h1`...`h6`) must not lock official `--kiwe-type-h*` font-size locally; H1-H6 scale belongs in Kiwe > Framework / Bricks Theme Style so changing the heading tag in the visual editor changes the heading scale. Do not emit notes/reports unless /document is present.',
				],
				'seam'              => [
	...
	private function conversion_contract(): array {
		return [
			...
			'fidelity' => [
				...
				'tokenPurityRule' => 'Bricks-native does not mean hardcoded-native. Element settings must not contain bare literal design lengths for padding/gaps/radius/min-height/type/shadows/transforms; use var(--kiwe-*), var(--seam-*), declared project variables, or real tokenized clamp() values backed by the Kiwe Framework profile. No-op clamps such as clamp(22px, 22px, 22px) are disguised literals and must fail. Bare var(--token) is also unsafe in template-upload styling; use var(--token, fallback) unless a verified Kiwe > Framework push runs before template import.',
				'editableImportRule' => 'Target public Bricks 2.3.x template import/runtime unless Site Graph confirms a newer public compatible version. Bricks My Templates can skip or remap global_classes when class names already exist, so full-page templates must include enough element-level native controls for rendered fidelity and visual-editor editability, while imported global_classes stay semantic/name-only to avoid duplicate-owner ghost styling.',
	...
	private function tool_use_rules(): array {
		return [
			...
			'For Bricks My Templates upload, do not rely on top-level globalVariables being installed from the same JSON. Every CSS variable consumed in native settings/global_classes needs a fallback, such as var(--nc-card-radius, 24px), unless the flow explicitly applies Kiwe > Framework before import. Store global variable names without leading --, use _widthMax/_widthMin/_heightMax/_heightMin for sizing, store border radius as _border.radius.top/right/bottom/left, keep var(...) font stacks out of _typography.font-family, store native color controls as Bricks color objects ({ raw: "var(--kiwe-color-surface, #fff)" }) rather than plain strings, and put gradients in _gradient instead of _background.color.',
			'Use a single visual owner for full-page My Templates output. Element-native controls are the render/edit owner; imported global_classes should be semantic/name-only, not a duplicate paint/layout/radius/spacing layer. Reusable project classes belong in the Framework profile push or a class-library artifact, not as duplicate styled classes inside the template upload.',
