...
final class Bricks_AI_Intelligence_Service {
	...
	private function conversion_contract(): array {
		return [
			...
			'fidelity' => [
				...
				'tokenPurityRule' => 'Bricks-native does not mean hardcoded-native. Element settings must not contain bare literal design lengths for padding/gaps/radius/min-height/type/shadows/transforms; use var(--kiwe-*), var(--seam-*), declared project variables, or real tokenized clamp() values backed by the Kiwe Framework profile. No-op clamps such as clamp(22px, 22px, 22px) are disguised literals and must fail. Inline fallbacks such as var(--token, fallback) are also unsafe because they hide missing Framework setup; require Kiwe > Framework push and use bare var(--token) in template-upload styling.',
	...
	private function tool_use_rules(): array {
		return [
			...
			'For Bricks My Templates upload, SeamFlow requires Kiwe > Framework profile push before template import. Every CSS variable consumed in native settings/global_classes must be bare, such as var(--nc-card-radius), never var(--nc-card-radius, 24px). Store global variable names without leading --, use _widthMax/_widthMin/_heightMax/_heightMin for sizing, store border radius as _border.radius.top/right/bottom/left, keep var(...) font stacks out of _typography.font-family, store native color controls as Bricks color objects ({ raw: "var(--kiwe-color-surface)" }) rather than plain strings, and put gradients in _gradient instead of _background.color.',
