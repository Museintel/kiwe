	...
	public function planning_packet( array $args = [] ): array {
		...
		return [
			...
			'plan'      => [
				...
				'conversion'        => [
					'rule' => 'When the human asks /convert /bricks, emit one token-pure native Bricks My Templates upload JSON at bricks-template/[page]-template-upload.json by default. Preserve approved layout and source semantics while mapping live data intent to Bricks-native elements/settings. Native settings and global_classes must consume Kiwe/Seam token variables, declared project variables, or real tokenized clamp() values for spacing, sizing, radius, type, shadow, transform offsets, and responsive layout; do not emit bare design lengths such as 28px, 2.35rem, or 390px, and do not use no-op clamps such as clamp(22px, 22px, 22px). For human My Templates upload, every CSS variable used in native settings/global_classes must include a safe fallback, e.g. var(--kiwe-color-surface, #fff8ef), because Bricks cannot be trusted to hydrate top-level globalVariables from the same JSON on every target site. Store native Bricks global variable names without leading -- because Bricks emits the prefix itself. Use Bricks source-backed sizing controls (_widthMax/_widthMin/_heightMax/_heightMin), not _maxWidth/_minWidth/_maxHeight/_minHeight; store border radius as _border.radius.top/right/bottom/left, not topLeft/topRight/bottomRight/bottomLeft; and do not place var(...) font stacks in _typography.font-family because Bricks quotes font-family output. Semantic Bricks heading elements (`name: heading` with `tag: h1`...`h6`) must not lock official `--kiwe-type-h*` font-size locally; H1-H6 scale belongs in Kiwe > Framework / Bricks Theme Style so changing the heading tag in the visual editor changes the heading scale. Do not emit notes/reports unless /document is present.',
				],
				'seam'              => [
