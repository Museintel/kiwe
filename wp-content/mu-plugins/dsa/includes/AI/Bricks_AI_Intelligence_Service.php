...
final class Bricks_AI_Intelligence_Service {
	...
	public function planning_packet( array $args = [] ): array {
		...
		return [
			...
			'plan'      => [
				...
				'conversion'        => [
					'rule' => 'When the human asks /convert /bricks, prepare the approved HTML/CSS/JS project for seam.kiwe by adding stable selectors and kiwe.bricks-bindings.v1 intent. Include both dynamic tags and query loops by default; /dynamictags or /queryloop narrows the pass to one binding type. Never emit Bricks JSON, a Framework Profile, or template-upload data from browser AI. Preserve approved layout, content, responsive behavior and JavaScript. Ground every site-specific field, tag, query type and identifier in SiteGraph.',
				],
				'seam'              => [
	...
	private function tool_use_rules(): array {
		return [
			'Use a scoped read-only SiteGraph connection or export before preparing site-specific Bricks bindings.',
			...
			'Use the current SEAM 8.1 command contract before browser-AI collaboration so the model sees Kiwe + Bricks + Seam boundaries in one packet.',
			'Use /convert /bricks only after the raw visual artifact is approved; output the updated raw project plus kiwe.bricks-bindings.v1, never Bricks JSON.',
