...
final class Bricks_AI_Intelligence_Service {
	...
	private function tool_use_rules(): array {
		return [
			...
			'For Bricks My Templates upload, do not rely on top-level globalVariables being installed from the same JSON. Every CSS variable consumed in native settings/global_classes needs a fallback, such as var(--nc-card-radius, 24px), unless the flow explicitly applies Kiwe > Framework before import. Store global variable names without leading --, use _widthMax/_widthMin/_heightMax/_heightMin for sizing, and keep var(...) font stacks out of _typography.font-family.',
