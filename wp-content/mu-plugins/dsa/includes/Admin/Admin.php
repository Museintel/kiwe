...
final class Admin {
	...
	public function assets( string $hook ): void {
		if ( ! in_array( $hook, [ 'toplevel_page_kiwe', 'kiwe_page_kiwe-auth', 'kiwe_page_kiwe-app', 'kiwe_page_kiwe-framework', 'kiwe_page_kiwe-sitegraph', 'kiwe_page_kiwe-ai', 'kiwe_page_kiwe-tokens', 'kiwe_page_kiwe-developer', 'kiwe_page_kiwe-database', 'kiwe_page_kiwe-dock', 'kiwe_page_kiwe-theme', 'kiwe_page_kiwe-games', 'kiwe_page_kiwe-links', 'kiwe_page_kiwe-search', 'kiwe_page_kiwe-menu', 'kiwe_page_kiwe-secure', 'kiwe_page_kiwe-email', 'kiwe_page_kiwe-woocommerce', 'kiwe_page_kiwe-analytics', 'kiwe_page_kiwe-abandoned-cart', 'kiwe_page_kiwe-bricks' ], true ) ) {
			return;
		}
	...
	public function save_settings(): void {
		...
		if ( ! in_array( $redirect_page, [ 'kiwe', 'kiwe-app', 'kiwe-haptic', 'kiwe-framework', 'kiwe-sitegraph', 'kiwe-ai', 'kiwe-dock', 'kiwe-theme', 'kiwe-games', 'kiwe-links', 'kiwe-search', 'kiwe-menu', 'kiwe-secure', 'kiwe-email', 'kiwe-woocommerce', 'kiwe-analytics', 'kiwe-abandoned-cart', 'kiwe-bricks', 'kiwe-developer', 'kiwe-database' ], true ) ) {
			$redirect_page = 'kiwe';
		}
	...
	public function export_site_graph(): void {
		...
		$payload = ( new Design_Context_Service( $service ) )->context( $args, true );
		$json             = wp_json_encode( $payload, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE );

		if ( ! $json ) {
			wp_safe_redirect( add_query_arg( 'site-graph-exported', 'encode-error', admin_url( 'admin.php?page=kiwe-sitegraph' ) ) );
			exit;
		}
	...
	public function save_site_graph_settings(): void {
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_die( esc_html__( 'You do not have permission to change Kiwe SiteGraph settings.', 'dsa' ) );
		}
		check_admin_referer( 'dsa_save_site_graph_settings' );

		$ai = $this->settings->get( 'ai', [] );
		$ai = is_array( $ai ) ? $ai : [];
		$ai['sitegraph_ai_enabled'] = ! empty( $_POST['sitegraph_ai_enabled'] );
		$this->settings->update( [ 'ai' => $ai ] );

		wp_safe_redirect( add_query_arg( 'site-graph-settings-saved', '1', admin_url( 'admin.php?page=kiwe-sitegraph' ) ) );
		exit;
	}

	public function sitegraph_staging_seed_preflight(): void {
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_die( esc_html__( 'You do not have permission to inspect a SiteGraph staging source.', 'dsa' ), esc_html__( 'Permission denied', 'dsa' ), [ 'response' => 403 ] );
		}
		check_admin_referer( 'dsa_sitegraph_staging_seed_preflight' );
		...
		try {
			$result = ( new Staging_Seed_Connection_Service() )->inspect( $source_url, $username, $password );
			unset( $password );
	...
	public function validate_binding_plan(): void {
		...
		wp_safe_redirect( add_query_arg( 'binding-report', $key, admin_url( 'admin.php?page=kiwe-sitegraph' ) ) );
		exit;
	}
	...
	public function gate_apply_stage(): void {
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_die(
				esc_html__( 'You do not have permission to build Kiwe pre-execution gates.', 'dsa' ),
				esc_html__( 'Permission denied', 'dsa' ),
				[ 'response' => 403 ]
			);
		}
		...
		check_admin_referer( 'dsa_gate_apply_stage_' . $stage_id );
		...
		$gate = ( new Pre_Execution_Gate_Service() )->evaluate(
			$stage,
			[
				'userId'    => get_current_user_id(),
				'createdAt' => gmdate( 'c' ),
		...
		if ( 'ready-for-final-executor-build' !== ( $gate['status'] ?? '' ) ) {
			wp_die(
				esc_html__( 'This apply candidate is not ready for the final executor gate.', 'dsa' ),
				esc_html__( 'Pre-execution gate blocked', 'dsa' ),
				[ 'response' => 409 ]
			);
		}

		$stager->attach_execution_gate( $stage_id, $gate );
	...
	public function build_controlled_executor(): void {
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_die(
				esc_html__( 'You do not have permission to build Kiwe controlled executors.', 'dsa' ),
				esc_html__( 'Permission denied', 'dsa' ),
				[ 'response' => 403 ]
			);
		}
		...
		check_admin_referer( 'dsa_build_controlled_executor_' . $stage_id );
		...
		$executor = ( new Controlled_Executor_Service() )->build(
			$stage,
			[
				'userId'    => get_current_user_id(),
				'createdAt' => gmdate( 'c' ),
		...
		if ( 'controlled-executor-skeleton-ready' !== ( $executor['status'] ?? '' ) ) {
			$stager->attach_controlled_executor( $stage_id, $executor );
			wp_die(
				esc_html__( 'Controlled executor skeleton found blockers. Review the staging row details before continuing.', 'dsa' ),
				esc_html__( 'Controlled executor blocked', 'dsa' ),
				[ 'response' => 409 ]
			);
		}
	...
	public function render_site_graph_page(): void {
		...
		if ( '' !== $seed_error_key ) {
			$seed_error = sanitize_text_field( (string) get_transient( 'dsa_staging_seed_error_' . $seed_error_key ) );
			delete_transient( 'dsa_staging_seed_error_' . $seed_error_key );
		}
		?>
		<div class="wrap dsa-admin">
			<h1><?php esc_html_e( 'Kiwe SiteGraph', 'dsa' ); ?></h1>
			<p class="description"><?php esc_html_e( 'The complete SiteGraph workspace: read-only discovery, SEAM Compiler calibration, binding validation, and the trusted apply chain. SiteGraph remains deterministic; optional AI is isolated behind the shared Kiwe AI broker.', 'dsa' ); ?></p>

			<?php if ( isset( $_GET['site-graph-settings-saved'] ) ) : ?>
				<div class="notice notice-success is-dismissible"><p><?php esc_html_e( 'SiteGraph settings saved.', 'dsa' ); ?></p></div>
			<?php endif; ?>
			...
			<section class="dsa-admin__panel">
				<h2><?php esc_html_e( 'SiteGraph AI isolation', 'dsa' ); ?></h2>
				<p><?php esc_html_e( 'SiteGraph never owns a provider or API key. When enabled, it may ask the shared Kiwe AI broker only for SiteGraph summaries or binding recommendations based on a redacted graph. The broker is stateless and cannot reuse Studio, SecureTrack, Bricks, visitor, order, payment, authentication, or prior-chat context.', 'dsa' ); ?></p>
				<div class="dsa-admin-token-summary">
					<div><strong><?php echo ! empty( $profile['enabled'] ) ? esc_html__( 'Enabled', 'dsa' ) : esc_html__( 'Disabled', 'dsa' ); ?></strong><span><?php esc_html_e( 'AI profile', 'dsa' ); ?></span></div>
					<div><strong><?php esc_html_e( 'Stateless', 'dsa' ); ?></strong><span><?php esc_html_e( 'requests', 'dsa' ); ?></span></div>
					<div><strong><?php esc_html_e( 'No secrets', 'dsa' ); ?></strong><span><?php esc_html_e( 'consumer access', 'dsa' ); ?></span></div>
					<div><strong><?php esc_html_e( 'Schema gate', 'dsa' ); ?></strong><span><?php esc_html_e( 'outputs', 'dsa' ); ?></span></div>
				</div>
				<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
					<input type="hidden" name="action" value="dsa_save_site_graph_settings">
					<?php wp_nonce_field( 'dsa_save_site_graph_settings' ); ?>
					<label><input type="checkbox" name="sitegraph_ai_enabled" value="1" <?php checked( ! empty( $ai_settings['sitegraph_ai_enabled'] ) ); ?>> <?php esc_html_e( 'Allow optional SiteGraph AI enrichment through the shared Kiwe AI broker', 'dsa' ); ?></label>
					<p class="description"><?php esc_html_e( 'Configure the provider once under Kiwe > AI or WordPress Settings > Connectors. Deterministic exports and validation do not require AI.', 'dsa' ); ?></p>
					<?php submit_button( __( 'Save SiteGraph AI policy', 'dsa' ), 'secondary', 'submit', false ); ?>
				</form>
			</section>
			...
			<section class="dsa-admin__panel" style="margin-top: 1rem;">
				...
				<?php if ( [] !== $seed_packages ) : ?>
					...
					<?php foreach ( array_slice( $seed_packages, 0, 4 ) as $seed_package ) : ?>
						...
						<tr>
							...
							<td><?php echo esc_html( (string) ( $seed_package['state'] ?? '' ) ); ?><br><small><?php echo $clean_ready ? esc_html__( 'Clean reconciliation ready', 'dsa' ) : esc_html__( 'Additive import only', 'dsa' ); ?></small></td>
							...
							<td>
								...
								<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
									...
									<?php if ( $clean_ready ) : ?><label><input type="checkbox" name="reconcilePublicData" value="1" checked> <?php esc_html_e( 'Reconcile public pages/posts and WooCommerce products to this source package; remove unmatched test records under the rollback baseline.', 'dsa' ); ?></label><br><?php endif; ?>
				...
				<?php if ( [] !== $seed_ledgers ) : ?>
					...
					<?php foreach ( array_slice( $seed_ledgers, 0, 5 ) as $seed_ledger ) : ?>
						<?php $seed_counts = is_array( $seed_ledger['resourceCounts'] ?? null ) ? $seed_ledger['resourceCounts'] : []; ?>
						<tr>
							<td><code><?php echo esc_html( (string) ( $seed_ledger['source']['origin'] ?? '' ) ); ?></code><br><small><?php echo esc_html( substr( (string) ( $seed_ledger['source']['revisionHash'] ?? '' ), 0, 12 ) ); ?></small></td>
							<td><?php echo esc_html( (string) ( $seed_ledger['state'] ?? '' ) ); ?></td>
							<td><?php echo esc_html( sprintf( 'Products %1$d · Content %2$d · Media %3$d', absint( $seed_counts['products'] ?? 0 ), absint( $seed_counts['content'] ?? 0 ), absint( $seed_counts['media'] ?? 0 ) ) ); ?></td>
							<td><?php echo empty( $seed_ledger['blockers'] ) ? esc_html__( 'Ready for reviewed import', 'dsa' ) : esc_html( sprintf( _n( '%d blocker', '%d blockers', count( (array) $seed_ledger['blockers'] ), 'dsa' ), count( (array) $seed_ledger['blockers'] ) ) ); ?><br><small><?php esc_html_e( 'Credentials not stored', 'dsa' ); ?></small></td>
							<td><?php echo esc_html( (string) ( $seed_ledger['createdAt'] ?? '' ) ); ?></td>
						</tr>
					<?php endforeach; ?>
					</tbody></table>
				<?php endif; ?>
			...
			<section class="dsa-admin__panel" style="margin-top: 1rem;">
				<h2><?php esc_html_e( 'SiteGraph workspace', 'dsa' ); ?></h2>
				<p><?php esc_html_e( 'Use this when an AI or web developer needs to turn an approved design into Bricks query loops, dynamic data, and Kiwe AppShell launchers for this exact WordPress site. SiteGraph is read-only and contains no secrets, visitor state, orders, payment data, or credentials.', 'dsa' ); ?></p>
				<div class="dsa-admin-token-summary">
					<div><strong><?php esc_html_e( 'Read-only', 'dsa' ); ?></strong><span><?php esc_html_e( 'SiteGraph', 'dsa' ); ?></span></div>
				...
				<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
					...
					<p class="dsa-admin-inline-fields">
						...
						<?php submit_button( __( 'Download SiteGraph JSON', 'dsa' ), 'secondary', 'submit', false ); ?>
					</p>
				</form>
				<p class="description"><?php esc_html_e( 'This is the one public-only AI handoff. It includes owner Design Context, approved design material, products, media, content, custom fields, taxonomies, Bricks capabilities and binding targets—never API keys, customers, orders, drafts, visitor state or filesystem data.', 'dsa' ); ?></p>
				<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" style="margin-top:.5rem;">
					...
					<p class="description"><?php esc_html_e( 'Content-free target profile for deterministic SEAM Compiler calibration: Bricks version, exact breakpoints, collision names and supported native capabilities.', 'dsa' ); ?></p>
				</form>
				<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" style="margin-top:.5rem;">
				...
				<form method="post" enctype="multipart/form-data" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
					<input type="hidden" name="action" value="dsa_validate_binding_plan"><?php wp_nonce_field( 'dsa_validate_binding_plan' ); ?>
					<p class="dsa-admin-inline-fields"><label><span><?php esc_html_e( 'Binding JSON', 'dsa' ); ?></span><input type="file" name="dsa_binding_file" accept="application/json,.json" required></label><label><span><?php esc_html_e( 'SiteGraph sample size', 'dsa' ); ?></span><select name="sampleLimit"><?php foreach ( [ 0, 4, 8, 16, 24 ] as $limit ) : ?><option value="<?php echo esc_attr( (string) $limit ); ?>" <?php selected( 8, $limit ); ?>><?php echo esc_html( (string) $limit ); ?></option><?php endforeach; ?></select></label><?php submit_button( __( 'Validate Binding Plan', 'dsa' ), 'secondary', 'submit', false ); ?></p>
				</form>
				...
				<ul class="ul-disc"><li><code>GET /status</code></li><li><code>GET /site-graph</code></li><li><code>POST /validate-bindings</code></li><li><code>POST /prepare-apply-plan</code></li><li><code>POST /stage-apply-plan</code> and <code>/stages/{stageId}/...</code></li></ul>
	...
	public function render_ai_page(): void {
		...
		<div class="wrap dsa-admin">
			...
			<p class="description"><?php esc_html_e( 'Configure AI once for all Kiwe services. WordPress 7 Connectors are preferred when available; Kiwe’s encrypted provider fallback remains available for older sites. Each consumer receives an isolated, stateless request profile and never receives provider credentials. SiteGraph operations live under Kiwe > SiteGraph.', 'dsa' ); ?></p>
			...
			<section class="dsa-admin__panel" style="margin-top: 1rem;">
				<h2><?php esc_html_e( 'SiteGraph has its own control plane', 'dsa' ); ?></h2>
				<p><?php esc_html_e( 'Exports, compiler pairing, binding validation, trusted staging, and SiteGraph AI isolation now live under Kiwe > SiteGraph.', 'dsa' ); ?></p>
				<p><a class="button button-secondary" href="<?php echo esc_url( admin_url( 'admin.php?page=kiwe-sitegraph' ) ); ?>"><?php esc_html_e( 'Open Kiwe SiteGraph', 'dsa' ); ?></a></p>
			</section>
		</div>
		<?php
	}
	...
	private function render_ai_companion_settings( array $ai_settings, array $companion_status, string $rest_base ): void {
		...
		$securetrack_broker = ( new AI_Broker_Service( $this->settings ) )->status( 'securetrack' );
		$securetrack_provider = is_array( $securetrack_broker['provider'] ?? null ) ? $securetrack_broker['provider'] : [];
		$securetrack_native_supported = 'none' !== $native_provider;
		$securetrack_broker_ready = ! empty( $securetrack_broker['profile']['enabled'] ) && ! empty( $securetrack_provider['nativeGeneration'] ) && ! empty( $securetrack_provider['configured'] );
		?>
						...
						<tr>
							...
							<td>
								...
								<p>
									...
									<?php if ( $securetrack_broker_ready ) : ?>
										<span style="color:#059669;font-weight:700"><?php esc_html_e( 'Shared broker ready', 'dsa' ); ?></span>
									<?php elseif ( ! $securetrack_native_supported ) : ?>
										<span style="color:#64748b;font-weight:700"><?php esc_html_e( 'Not configured', 'dsa' ); ?></span>
									<?php else : ?>
										<span style="color:#d97706;font-weight:700"><?php esc_html_e( 'Broker not ready', 'dsa' ); ?></span>
									<?php endif; ?>
				...
				<?php foreach ( array_slice( $records, 0, 5 ) as $record ) : ?>
					...
					<tr<?php echo ( $id === $active_stage || $id === $active_proof || $id === $active_auth || $id === $active_gate || $id === $active_preview || $id === $active_confirm || $id === $active_fresh || $id === $active_rollback || $id === $active_target || $id === $active_capture || $id === $active_inspection || $id === $active_shell || $id === $active_save || $id === $active_executor || $id === $active_adapter || $id === $active_post_verify ) ? ' class="is-active"' : ''; ?>>
						...
						<td>
							...
							<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" style="margin-top: .35rem;">
								<input type="hidden" name="action" value="dsa_gate_apply_stage">
								<input type="hidden" name="stageId" value="<?php echo esc_attr( $id ); ?>">
							...
							<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" style="margin-top: .35rem;">
								<input type="hidden" name="action" value="dsa_build_controlled_executor">
								<input type="hidden" name="stageId" value="<?php echo esc_attr( $id ); ?>">
								<?php wp_nonce_field( 'dsa_build_controlled_executor_' . $id ); ?>
	...
	private function render_pre_execution_gate_details( array $gate ): void {
		if ( [] === $gate ) {
			return;
		}
		$gates    = isset( $gate['gates'] ) && is_array( $gate['gates'] ) ? $gate['gates'] : [];
		$blockers = isset( $gate['blockers'] ) && is_array( $gate['blockers'] ) ? $gate['blockers'] : [];
		$required = isset( $gate['requiredBeforeMutation'] ) && is_array( $gate['requiredBeforeMutation'] ) ? $gate['requiredBeforeMutation'] : [];
		?>
		<p><strong><?php esc_html_e( 'Pre-execution gate', 'dsa' ); ?></strong></p>
		<div class="dsa-admin-token-summary">
			<div><strong><?php echo esc_html( (string) ( $gate['status'] ?? '' ) ); ?></strong><span><?php esc_html_e( 'status', 'dsa' ); ?></span></div>
			<div><strong><code><?php echo esc_html( substr( (string) ( $gate['id'] ?? '' ), 0, 24 ) ); ?></code></strong><span><?php esc_html_e( 'gate', 'dsa' ); ?></span></div>
			<div><strong><?php echo esc_html( (string) count( $blockers ) ); ?></strong><span><?php esc_html_e( 'blockers', 'dsa' ); ?></span></div>
			<div><strong><?php esc_html_e( 'No', 'dsa' ); ?></strong><span><?php esc_html_e( 'mutates now', 'dsa' ); ?></span></div>
		</div>
		<?php if ( [] !== $gates ) : ?>
			<ul class="ul-disc">
				<?php foreach ( $gates as $item ) : ?>
					<?php
					if ( ! is_array( $item ) ) {
						continue;
					}
		...
		<?php endif; ?>
		<?php if ( [] !== $required ) : ?>
			<p><strong><?php esc_html_e( 'Still required before any real mutation', 'dsa' ); ?></strong></p>
			<ul class="ul-disc">
	...
	private function render_controlled_executor_details( array $executor ): void {
		if ( [] === $executor ) {
			return;
		}
		$blockers  = isset( $executor['blockers'] ) && is_array( $executor['blockers'] ) ? $executor['blockers'] : [];
		$gates     = isset( $executor['gates'] ) && is_array( $executor['gates'] ) ? $executor['gates'] : [];
		$interface = isset( $executor['executionInterface'] ) && is_array( $executor['executionInterface'] ) ? $executor['executionInterface'] : [];
		$checklist = isset( $executor['preMutationChecklist'] ) && is_array( $executor['preMutationChecklist'] ) ? $executor['preMutationChecklist'] : [];
		$ops       = isset( $executor['approvedOperationIds'] ) && is_array( $executor['approvedOperationIds'] ) ? $executor['approvedOperationIds'] : [];
		?>
		<p><strong><?php esc_html_e( 'Controlled executor skeleton', 'dsa' ); ?></strong></p>
		<div class="dsa-admin-token-summary">
		...
		<p class="description"><?php esc_html_e( 'This skeleton defines the future controlled adapter interface. It does not save Bricks, update WordPress content, publish, modify WooCommerce, or execute adapter code.', 'dsa' ); ?></p>
		<ul class="ul-disc">
			<li><?php esc_html_e( 'Approval:', 'dsa' ); ?> <code><?php echo esc_html( (string) ( $executor['finalSaveApprovalId'] ?? '' ) ); ?></code></li>
			<li><?php esc_html_e( 'Shell:', 'dsa' ); ?> <code><?php echo esc_html( (string) ( $executor['minimalAdapterShellId'] ?? '' ) ); ?></code></li>
			<li><?php esc_html_e( 'Target:', 'dsa' ); ?> <code><?php echo esc_html( (string) ( $executor['targetPostId'] ?? 0 ) ); ?></code></li>
			<li><?php esc_html_e( 'Strategy:', 'dsa' ); ?> <code><?php echo esc_html( (string) ( $executor['selectedStrategyId'] ?? '' ) ); ?></code></li>
			<li><?php esc_html_e( 'Plan hash:', 'dsa' ); ?> <code><?php echo esc_html( substr( (string) ( $executor['planHash'] ?? '' ), 0, 24 ) ); ?></code></li>
		</ul>
		<?php if ( [] !== $gates ) : ?>
			<p><strong><?php esc_html_e( 'Executor gates', 'dsa' ); ?></strong></p>
			<ul class="ul-disc">
				<?php foreach ( $gates as $gate ) : ?>
					<?php if ( is_array( $gate ) ) : ?>
						<li><code><?php echo esc_html( (string) ( $gate['status'] ?? '' ) ); ?></code> <?php echo esc_html( (string) ( $gate['label'] ?? $gate['id'] ?? '' ) ); ?></li>
		...
		<?php endif; ?>
		<?php if ( [] !== $checklist ) : ?>
			<p><strong><?php esc_html_e( 'Required before any future mutation', 'dsa' ); ?></strong></p>
			<ul class="ul-disc">
	...
	private function render_app_audience(): void {
		...
		$email_health = $this->email ? $this->email->diagnostics() : [ 'fallback_ready' => false ];
		$phonekey_ready = function_exists( 'pk_whatsapp_notification_ready' ) && pk_whatsapp_notification_ready();
			...
			<div class="dsa-lpm-card">
				...
				<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
					...
					<?php foreach ( $channel_labels as $channel => $label ) : $count = (int) ( $summary['registeredChannels'][ $channel ] ?? 0 ); ?>
						<button class="button button-primary" type="submit" name="channel" value="<?php echo esc_attr( $channel ); ?>" <?php disabled( 0 === $count || ! $this->notification_campaigns || ! $this->notification_campaigns->channel_ready( $channel ) ); ?>><?php echo esc_html( sprintf( __( 'Send as %1$s (%2$d)', 'dsa' ), $label, $count ) ); ?></button>
					<?php endforeach; ?>
					</p>
...
&lt;p id="theme-status" aria-live="polite"&gt;&lt;/p&gt;</code></pre>
	...
	private function redirect_binding_plan_error( string $code ): void {
		wp_safe_redirect( add_query_arg( 'binding-plan', sanitize_key( $code ), admin_url( 'admin.php?page=kiwe-sitegraph' ) ) );
		exit;
	}
	...
	private function sanitize_ai_settings( $input, array $current ): array {
		...
		return [
			...
			'sitegraph_ai_enabled'      => ! empty( $input['sitegraph_ai_enabled'] ?? ( $current['sitegraph_ai_enabled'] ?? false ) ),
	...
	private function save_securetrack_ai_settings( $input, array $ai_settings = [] ): void {
		...
		$native_provider = sanitize_key( (string) ( $ai_settings['native_provider'] ?? 'none' ) );
		$provider        = in_array( $native_provider, [ 'wordpress_ai_client', 'openai_compatible', 'gemini', 'groq', 'xai' ], true ) ? $native_provider : 'none';
		...
		$next['v2_ai_key']           = '';
		// SecureTrack is a broker consumer and never receives or duplicates the shared provider secret.
		$next['v2_ai_key_enc']       = '';
		...
		if ( function_exists( 'stp_ai_status' ) && ( $provider !== ( $old['v2_ai_provider'] ?? 'none' ) || $model !== ( $old['v2_ai_model'] ?? '' ) || '' !== (string) ( $raw_old['v2_ai_key_enc'] ?? '' ) ) ) {
			stp_ai_status(
				[
					'connected' => 0,
					'provider'  => $provider,
					'model'     => $model,
					'message'   => 'none' === $provider ? 'SecureTrack is using local-only review.' : 'SecureTrack is registered with the shared Kiwe AI broker; no provider key was copied.',
				]
			);
		}
	}
	...
	private function sanitize_abandoned_cart_settings( $input, array $current ): array {
		...
		return [
			...
			'automatic_email_enabled'  => ! empty( $input['automatic_email_enabled'] ),
			'automatic_whatsapp_enabled' => ! empty( $input['automatic_whatsapp_enabled'] ),
			'guest_email_reminders_enabled' => ! empty( $input['guest_email_reminders_enabled'] ),
			...
			'automation_batch_limit'    => max( 1, min( 250, absint( $input['automation_batch_limit'] ?? ( $current['automation_batch_limit'] ?? 25 ) ) ) ),
