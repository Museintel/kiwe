...
final class Onboarding_Service {
	...
	public function assets( string $hook ): void {
		...
		wp_localize_script( 'kiwe-onboarding', 'KIWE_ONBOARDING', [ 'chooseImage' => __( 'Choose image', 'dsa' ), 'useImage' => __( 'Use this image', 'dsa' ) ] );
	}
		...
		<div class="wrap kiwe-onboarding" data-kiwe-onboarding>
			...
			<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" data-kiwe-onboarding-form>
				...
				<section class="kiwe-onboarding__panel" data-kiwe-step="2" hidden>
					...
					<div class="kiwe-fields kiwe-fields--two"><label><span><?php esc_html_e( 'Public phone number', 'dsa' ); ?> *</span><input required type="tel" name="context[contact][phone]" value="<?php echo esc_attr( $p['contact']['phone'] ); ?>"></label><label><span><?php esc_html_e( 'Public email address', 'dsa' ); ?> *</span><input required type="email" name="context[contact][email]" value="<?php echo esc_attr( $p['contact']['email'] ); ?>"></label><label><span><?php esc_html_e( 'WhatsApp number', 'dsa' ); ?></span><input type="tel" name="context[contact][whatsapp]" value="<?php echo esc_attr( $p['contact']['whatsapp'] ); ?>"></label><label><span><?php esc_html_e( 'Timezone', 'dsa' ); ?></span><input name="context[localization][timezone]" value="<?php echo esc_attr( $p['localization']['timezone'] ); ?>" data-kiwe-timezone></label></div>
				...
				<section class="kiwe-onboarding__panel" data-kiwe-step="3" hidden>
					<div class="kiwe-onboarding__intro"><span>04</span><div><h2><?php esc_html_e( 'Brand feeling', 'dsa' ); ?></h2><p><?php esc_html_e( 'Optional and intentionally simple. Pick a mood first, then up to three familiar colors. These guide SEAM; they do not silently install a framework.', 'dsa' ); ?></p></div></div>
					<h3><?php esc_html_e( 'Overall mood', 'dsa' ); ?></h3><div class="kiwe-choice-grid"><?php foreach ( [ 'pastel'=>'Soft pastel', 'vibrant'=>'Bright & energetic', 'muted'=>'Calm & muted', 'natural'=>'Earthy & natural', 'dark'=>'Rich & dark', 'light'=>'Fresh & light', 'luxury'=>'Elegant & premium', 'playful'=>'Playful', 'minimal'=>'Minimal' ] as $value => $label ) : ?><label><input type="radio" name="context[brand][tone]" value="<?php echo esc_attr( $value ); ?>" <?php checked( $p['brand']['tone'], $value ); ?>><span><?php echo esc_html( $label ); ?></span></label><?php endforeach; ?><label><input type="radio" name="context[brand][tone]" value="" <?php checked( $p['brand']['tone'], '' ); ?>><span><?php esc_html_e( 'Let the designer decide', 'dsa' ); ?></span></label></div>
					<div class="kiwe-color-roles"><?php foreach ( [ 'brand'=>'Main brand color', 'accent'=>'Accent color', 'support'=>'Supporting color' ] as $role => $label ) $this->color_role( $role, $label, $p['brand']['colors'] ); ?></div>
					<label class="kiwe-full"><span><?php esc_html_e( 'Any visual likes, dislikes or references?', 'dsa' ); ?></span><textarea name="context[brand][notes]" rows="3"><?php echo esc_textarea( $p['brand']['notes'] ); ?></textarea></label>
				</section>
				...
				<section class="kiwe-onboarding__panel" data-kiwe-step="4" hidden>
					...
					<h3><?php esc_html_e( 'Pages still planned', 'dsa' ); ?></h3><div class="kiwe-page-plan"><?php for ( $i = 0; $i < 8; $i++ ) : $page = $p['contentPlan']['plannedPages'][ $i ] ?? [ 'name'=>'', 'visibility'=>'primary' ]; ?><div><input name="context[contentPlan][plannedPages][<?php echo esc_attr( (string) $i ); ?>][name]" value="<?php echo esc_attr( $page['name'] ); ?>" placeholder="About us"><select name="context[contentPlan][plannedPages][<?php echo esc_attr( (string) $i ); ?>][visibility]"><option value="primary" <?php selected( $page['visibility'], 'primary' ); ?>><?php esc_html_e( 'Primary', 'dsa' ); ?></option><option value="secondary" <?php selected( $page['visibility'], 'secondary' ); ?>><?php esc_html_e( 'Secondary', 'dsa' ); ?></option></select></div><?php endfor; ?></div>
				</section>
				...
				<section class="kiwe-onboarding__panel" data-kiwe-step="5" hidden data-kiwe-commerce-panel>
					...
					<div class="kiwe-fields kiwe-fields--two" data-kiwe-commerce-fields><label><span><?php esc_html_e( 'Expected number of products', 'dsa' ); ?></span><input type="number" min="0" name="context[commerce][expectedProductCount]" value="<?php echo esc_attr( (string) $p['commerce']['expectedProductCount'] ); ?>"></label><label><span><?php esc_html_e( 'Currency', 'dsa' ); ?></span><input maxlength="3" name="context[commerce][currency]" value="<?php echo esc_attr( $p['commerce']['currency'] ); ?>"></label><label><span><?php esc_html_e( 'Lowest expected price', 'dsa' ); ?></span><input type="number" min="0" step="0.01" name="context[commerce][expectedPriceRange][min]" value="<?php echo esc_attr( (string) $p['commerce']['expectedPriceRange']['min'] ); ?>"></label><label><span><?php esc_html_e( 'Highest expected price', 'dsa' ); ?></span><input type="number" min="0" step="0.01" name="context[commerce][expectedPriceRange][max]" value="<?php echo esc_attr( (string) $p['commerce']['expectedPriceRange']['max'] ); ?>"></label><label><span><?php esc_html_e( 'Shipping model', 'dsa' ); ?></span><select name="context[commerce][shippingModel]"><?php foreach ( [ ''=>'Not decided', 'free'=>'Free shipping', 'flat'=>'Flat charge', 'calculated'=>'Calculated by location', 'pickup'=>'Pickup', 'mixed'=>'A mix of methods' ] as $value=>$label ) : ?><option value="<?php echo esc_attr( $value ); ?>" <?php selected( $p['commerce']['shippingModel'], $value ); ?>><?php echo esc_html( $label ); ?></option><?php endforeach; ?></select></label><label><span><?php esc_html_e( 'Typical shipping charge', 'dsa' ); ?></span><input type="number" min="0" step="0.01" name="context[commerce][typicalShippingCharge]" value="<?php echo esc_attr( (string) $p['commerce']['typicalShippingCharge'] ); ?>"></label></div>
				...
				<section class="kiwe-onboarding__panel" data-kiwe-step="6" hidden>
					...
					<div class="kiwe-review-grid"><article><h3><?php esc_html_e( 'WordPress', 'dsa' ); ?></h3><p><?php esc_html_e( 'Site title, tagline, logo, site icon, timezone, indexing preference and native XML sitemap intent.', 'dsa' ); ?></p></article><article><h3><?php esc_html_e( 'WooCommerce', 'dsa' ); ?></h3><p><?php esc_html_e( 'Store address, country, currency and safe tax switches when this is a store.', 'dsa' ); ?></p></article><article><h3><?php esc_html_e( 'Kiwe', 'dsa' ); ?></h3><p><?php esc_html_e( 'Public phone/email, design brief, page search roles and product-plan context.', 'dsa' ); ?></p></article><article><h3><?php esc_html_e( 'SiteGraph', 'dsa' ); ?></h3><p><?php esc_html_e( 'One bounded evidence packet combining this owner context with configured public content, media, products, custom fields and taxonomies.', 'dsa' ); ?></p></article></div>
				</section>
	...
	public function handle_save(): void {
		...
		if ( is_wp_error( $result ) ) {
			...
			wp_safe_redirect( admin_url( 'admin.php?page=kiwe-onboarding&incomplete=1' ) ); exit;
		}
