...
The current phase-close proof lives at `site-graph-system/release-proof-6.52.md`. Use it after uploading the folder-based MU package to staging. It lists the expected local package commands, API proof routes, WordPress 7 ability proof, dynamic handoff proof, accessibility/dark-mode proof, browser smoke checks, and Bricks native template-upload validator checks for this Site Graph + internal AI phase.
