...
Batch 44 promotes National Chikki v3 review findings into core AppShell/toolkit capabilities. Kiwe Dock now supports custom URL-only dock links and a selectable dock `focus_item` for the emphasized split-dock center. The launcher contract stays singular: page/header controls open AppShell modules through canonical `data-dsa-open-module`. DSA Menu context now prefers public Seam section landmarks (`data-role="section"` or `.seam-section`) with a stable `id` and standard `aria-label`, `aria-labelledby`, or visible heading text before falling back to configured heading levels, so website sections such as Heritage, Bestsellers, or Limca Record can feed the AppShell menu without inventing Kiwe-only page attributes. The AI toolkit, Bricks capability JSON, handoff modes, slots map, and generated browser contexts now document these hooks and Kiwe settings profile keys so future combined handoffs output AppShell-ready pages/settings instead of static mock icons or invented menu context.
