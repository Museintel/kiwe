...
Batch 44 promotes National Chikki v3 review findings into core AppShell/toolkit capabilities. Kiwe Dock now supports custom URL-only dock links, a selectable dock `focus_item` for the emphasized split-dock center, and readable page/header module launch aliases (`data-kiwe-open`, `data-dsa-open`) alongside canonical `data-dsa-open-module`. DSA Menu context now prefers explicit page section landmarks (`data-kiwe-menu-section` with `data-kiwe-menu-label`) before falling back to configured heading levels, so website sections such as Heritage, Bestsellers, or Limca Record can feed the AppShell menu without forcing matching H1/H2/H3 text. The AI toolkit, Bricks capability JSON, handoff modes, slots map, and generated browser contexts now document these hooks and Kiwe settings profile keys so future combined handoffs output AppShell-ready pages/settings instead of static mock icons or invented menu context.
