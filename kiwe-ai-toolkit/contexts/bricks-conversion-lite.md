...
- Variable availability is part of the Bricks conversion contract. Bricks My Templates uploads import `global_classes`, but the template upload path cannot be trusted to install every `globalVariables` entry from the same JSON on every target site/version. Therefore every CSS variable consumed by Bricks-native element settings or `global_classes` must include a safe fallback, for example `var(--kiwe-color-surface, #fff8ef)`, `var(--nc-promo-min, clamp(220px, calc(135.54px + 17.78vw), 390px))`, or `minmax(var(--sf-hero-side-min, 260px), .65fr)`. A bare `var(--token)` in native Bricks styling is a blocking failure unless the delivery route explicitly uses a verified Kiwe > Framework push before template import. Top-level `globalVariables` are still useful, but they are not proof that the page will render after a human My Templates upload.
...
- Every project class that is expected to style imported content should also be declared in `settings.tokens.project.classes` when it is part of the reusable project design system. The Bricks template may still carry `global_classes`, but collision-safe profile-pushed classes make imports less dependent on Bricks' class hydration behavior.
...
The native Bricks template export must also carry the editable design itself. For full-page templates, Bricks-native element settings/global classes should include the layout and visual controls, and those controls must point back to Kiwe/Seam tokens rather than frozen pixel/rem values. A template whose `content` array is present but whose styling lives mainly in `pageSettings.customCss` is not a valid production-grade conversion, because it can import structurally and still render wrongly after insertion. A template whose design relies mainly on `global_classes` hydration is also not production-grade: Bricks My Templates can skip or remap global classes when the same class names already exist on the target site, so enough element-level native controls must remain on full-page templates for resilient rendering and visual-editor editability. Likewise, a template that relies on top-level `globalVariables` without fallbacks is not production-grade for human My Templates upload; Bricks may not hydrate those variables during import, so native settings/global classes must either use `var(--token, fallback)` or the workflow must require Kiwe > Framework to be pushed before import.
