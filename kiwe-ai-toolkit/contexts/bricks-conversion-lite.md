...
- Fail the conversion source if project CSS redefines bare Seam framework selectors such as `.seam-horizontal-rail`, `.seam-card`, or `[data-flow="reel"]`. Seam selectors are shared vocabulary. Visual rules must live on project-owned classes before conversion, otherwise Bricks imports can inherit rail/card/grid behavior in the wrong place.
