...
- Bento/campaign/editorial grids, CSS grid columns/rows/spans, media-query layout changes, and Bricks responsive layout overrides have `fidelity.responsiveIntent` entries naming breakpoint/range, source selector, mapped Bricks element IDs, Bricks controls, and intended grid/flex behavior. Bricks 2.4 responsive control keys use `controlKey:breakpoint`; layout elements (`container`, `div`, `section`, `block`) must use `_direction` / `_direction:<breakpoint>` for flex direction. `_flexDirection` is valid only for non-nestable Bricks elements. Audit grid controls, `_cssCustom:<breakpoint>`, and custom breakpoint keys instead of assuming only default viewport names exist.
- Fail Bricks conversions that park project-wide variables, `@media` rules, bento/campaign CSS, or global page selectors inside one element's `_cssCustom`. Use `pageSettings.customCss`, global classes/variables, or native Bricks controls; element `_cssCustom` is too fragile for full-page responsive conversion fidelity.
- Bricks conversion must be editable in the Bricks visual editor, not merely visually rendered through a CSS dump. Prefer native Bricks controls/global classes/global variables for typography, colors/backgrounds/gradients, borders/radii, shadows, transforms, filters, transitions, spacing, sizing, grid/flex, responsive direction, alignment, query loops, conditions, interactions, and dynamic tags. If substantial custom CSS remains, `fidelity.nativeStyleIntent` must prove which selectors were mapped to editable Bricks-native controls and which rules remain explicit custom-CSS exceptions.
...
- Bricks conversion must declare `target.importMethod` as one of `review-only`, `bricks-clipboard-json`, `bricks-admin-template-upload`, or `kiwe-staging-executor`. `kiwe-bricks-conversion.json` is a Kiwe audit/executor envelope, not a Bricks "My Templates" upload file. Full pages and large sections should use `bricks-admin-template-upload` or `kiwe-staging-executor`, not clipboard JSON. If the output claims Bricks admin template upload, require a separate native Bricks template export JSON with non-empty `title`, `templateType`, and non-empty `content`, `header`, or `footer`; otherwise reject it because Bricks imports it as `(no title)` or inserts with "This template has no data". For template-upload delivery, require Bricks template dependency rows under `global_classes` when imported elements rely on global class styling; copied-elements `globalClasses` alone is insufficient for this path.
