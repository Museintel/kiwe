...
- importable `theme.css` contains anonymous raw pixel literals such as `22px`, `35px`, `1px`, or `999px`. Fail these even when they appear inside CSS custom property declarations. Concrete base values belong in `theme-package.json settings.tokens` or Kiwe core token registries; importable theme CSS should consume official `--kiwe-*`, documented `--kiwe-theme-*`, or Kiwe/DSA Geometry Engine variables instead;
