...
- Bento/campaign/editorial grids, CSS grid columns/rows/spans, media-query layout changes, and Bricks responsive layout overrides have `fidelity.responsiveIntent` entries naming breakpoint/range, source selector, mapped Bricks element IDs, and intended grid/flex behavior. Bricks 2.4 responsive control keys use `controlKey:breakpoint`; audit both `_direction:<breakpoint>` and `_flexDirection:<breakpoint>`, grid controls, `_cssCustom:<breakpoint>`, and custom breakpoint keys instead of assuming only default viewport names exist.
