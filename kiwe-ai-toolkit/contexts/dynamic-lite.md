...
The controlled executor skeleton defines the future adapter interface only. The Bricks controlled adapter plan maps approved operation IDs to deterministic Bricks/Kiwe adapter instructions. These artifacts are valid only while they still report `adapterCanSaveNow: false`, `actualSaveExecuted: false`, and `mayExecuteMutationNow: false` until post-apply verification and rollback proof exist.
