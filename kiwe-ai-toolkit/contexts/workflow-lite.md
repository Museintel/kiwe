...
- Use official Kiwe variables implied by token names, for example `color-brand` -> `--kiwe-color-brand` and `space-md` -> `--kiwe-space-md`. Do not invent non-canonical or unregistered reserved variables such as `--kiwe-color-primary`, `--kiwe-type-2xs`, `--kiwe-letterspace-eyebrow`, `--kiwe-radius-pill`, or `--seam-color-primary`. If a project needs a value outside the universal registry, declare it in `settings.tokens.project.variables` with a project prefix instead of minting a new `--kiwe-*` name.
...
- Target the public Bricks 2.3.x My Templates importer/runtime unless Site Graph or the human explicitly confirms a newer public compatible Bricks version. Do not emit unreleased/beta `2.4` metadata as production proof.
...
External converter outputs are intake/scaffold only. Bricks' native HTML/CSS converter, Code2Bricks-style tools, and third-party Bricks skills can help discover Bricks element/control shapes, but final Kiwe `/convert /bricks` must normalize representable design into native Bricks elements, controls, Framework/profile variables, Kiwe attributes, interactions, conditions, and query intent. A Code element with CSS, JavaScript, HTML, PHP, `executeCode`, or equivalent runtime/custom-code settings is review-only and must fail the normal production lane unless it is explicitly marked as an unsupported/manual-review exception.

Native-first does not mean hardcoded-native. Template-upload handoffs must not depend on `pageSettings.customCss` for ordinary design, because inserted templates can lose that page CSS or be controlled by stale target-page CSS. For full-page Bricks My Templates uploads, put typography, layout, responsive grid/flex, backgrounds, borders, shadows, sizing, spacing, and alignment into Bricks element-native settings as the render/edit owner. Use global variables for token definitions. Keep imported `global_classes` semantic/name-only unless the human explicitly asked for a class-library artifact; reusable styled project classes belong in the Framework profile push, not duplicated inside every page template. Native Bricks values must still consume the Kiwe/Seam Framework token layer: use registered `var(--kiwe-*)`, registered `var(--seam-*)`, declared project variables, or real tokenized `clamp(...)` values for spacing, sizing, radius, type, shadow, transform offsets, responsive layout controls, and every component color/paint value. Do not emit hardcoded values such as `padding: 28px`, `_border.radius: 24px`, `_heightMin: 390px`, `_typography.font-size: 2.35rem`, or `_rowGap: 20px` in element settings; map them to Framework tokens/project variables backed by `kiwe-framework-profile.json`. Do not emit direct component colors such as `color: #fff`, `_background.color.raw: #8deae5`, `linear-gradient(#201b18, #514238)`, `rgba(255,255,255,.11)`, or `--pack-bg: #f5b942`; map them to official Kiwe/Seam variables or declared project variables. Literal colors and primitive values are allowed only in the Framework/global variable definition layer. Do not use CSS-variable fallbacks in Bricks render-owner settings or template-local variable records: `var(--token, fallback)` is a blocking failure because it hides missing Framework/profile setup and causes ghost styling. Use bare `var(--token)` in Bricks styling, and require a verified Kiwe > Framework push before template import. Native Bricks `global_variables`/`globalVariables` names must be stored without leading `--` (`nc-app-max`, not `--nc-app-max`) because Bricks adds the CSS custom-property prefix during compilation; leading dashes compile to disconnected `----token` variables. Reserved `--kiwe-*` and `--seam-*` variables must be verified against the current universal token registry; do not mint them inside a page template. Use Bricks source-backed sizing controls such as `_widthMax`/`_widthMin`/`_heightMax`/`_heightMin`, not `_maxWidth`/`_minWidth`/`_maxHeight`/`_minHeight`; store native background/border/typography colors as Bricks color objects with `raw`, `rgb`, or `hex`; put gradients in `_gradient`, not `_background.color`; store Bricks border radius as `_border.radius.top`, `_border.radius.right`, `_border.radius.bottom`, and `_border.radius.left` because Bricks maps those four side keys to CSS corners during compilation; never use CSS corner keys such as `topLeft`, `topRight`, `bottomRight`, or `bottomLeft`; do not put `var(...)` font stacks in `_typography.font-family` because Bricks quotes that value; and do not put official H1-H6 token font-size locks directly on semantic Bricks Heading elements. A `heading` element tagged `h3` must inherit H3 sizing from Kiwe > Framework / Bricks Theme Style, so changing the tag to `h2` or `h4` in the visual editor changes the heading scale. Component-specific display/card titles may use project classes/tokens, but they must not mask semantic heading-level behavior. No-op clamps such as `clamp(22px, 22px, 22px)` are disguised literals and must fail. Token definitions may use plain values when their behavior role is a named `fixed-primitive`, `geometry-input`, `content-limit`, or `responsive-guard`; Bricks/page output still must consume the named token rather than copy the plain value. Tokenization ladder: first use an official Kiwe/Seam token when meaning and property domain match; second use a declared project token for stable art direction; third use a real fluid clamp only when the source proves different responsive values for the same property. Kiwe clamp math is `slope = (maxValue - minValue) / (maxViewport - minViewport) * 100`, `intercept = minValue - (slope / 100 * minViewport)`, then `clamp(minValue, calc(intercept + slope * 1vw), maxValue)`. CLI/MCP-capable tools may run `kiwe fluid-clamp --min 220px --max 390px --min-vw 478 --max-vw 1440`. `fidelity.responsiveIntent` or embedded `kiwe.fidelity.responsiveIntent` is required whenever the conversion contains bento/campaign/editorial grids, CSS grid placement, media-query layout behavior, or Bricks breakpoint layout overrides. Bricks responsive controls are stored as `controlKey:breakpoint`, including `_direction:<breakpoint>`, grid controls, `_cssCustom:<breakpoint>`, and custom site breakpoint keys. Bricks layout elements (`container`, `div`, `section`, `block`) use `_direction` / `_direction:<breakpoint>` for flex direction; `_flexDirection` is only for non-nestable elements. CSS-heavy or mappable-CSS-heavy conversions must include native style proof showing ordinary visual rules became editable Bricks controls/global variables before any custom-CSS exceptions; common layout/visual properties should be emitted as controls such as `_display`, `_direction`, `_justifyContent`, `_alignItems`, `_flexWrap`, `_columnGap`, `_rowGap`, `_gridTemplateColumns`, `_gridItemColumnSpan`, `_typography`, `_background`, `_gradient`, `_border`, `_boxShadow`, `_transform`, `_cssFilters`, and `_cssTransition`. Bricks My Templates can skip or remap `global_classes` when the same class names already exist on the target site, so large full-page templates must remain visually resilient with enough element-level native controls; do not rely mainly on class hydration. Also do not duplicate styling into both element-native controls and styled imported `global_classes`, because that creates ghost styling where a designer removes/changes one visible layer but the same color/radius/spacing remains active from another layer.
...
- large full-page templates keep the rendered design resilient even if Bricks skips/remaps existing global classes: element-level native controls per element must be sufficient, and class-only/global-class-dependent elements must stay below the audit limit;
- Bricks Code elements do not contain CSS, JavaScript, HTML, PHP, `executeCode`, or equivalent runtime/custom-code settings unless the artifact is explicitly review-only/unsupported; external converter output that stores normal design in Code elements is not a completed Kiwe conversion;
