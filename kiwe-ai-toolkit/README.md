...
For vendor-neutral browser AI, IDE and tool access, first open `/wp-json/dsa/v1/ai/client-manifest` or import `/wp-json/dsa/v1/ai/openapi.json`. Prefer the short-lived connection package downloaded from `Kiwe > SiteGraph`; its `kiwe_task_...` capsule is limited to public SiteGraph data plus read/convert/validate operations and cannot enter staging or mutation routes. The `/wp-json/dsa/v1/ai` URL is only a namespace. A client must use a discovered route and configure the bearer secret outside model conversation context. Permanent `kiwe_ai_...` keys remain available for explicitly authorized staging chains and must use the smallest scopes possible.
