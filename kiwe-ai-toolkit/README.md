...
Use `/list` to see the command vocabulary and stop. Use `/fix` to repair an existing failed artifact lane without starting over. Use `/usesitegraph /for <target>` for SiteGraph-grounded work and `/usebrickscontext /for <target>` for general Bricks dynamic/query planning without site data. SiteGraph is the single AI handoff and already contains public Design Context plus approved design material, products, media, standard/custom content, fields, taxonomies, Kiwe merchandising and target capabilities. Append `/nonai` when attaching the downloadable `kiwe.site-graph.v1` file in an ordinary/free browser chat. Add universal entity/field scopes when needed; `/usesitegraph /for /previewdata /products /titles /images /nonai` changes only product names and images. The command tokens carry the complete general contract; the human should not have to restate it as a long prompt.
