...
- `website` â€” normal WordPress/Bricks page or website using Seam Framework.
- `theme` â€” Kiwe DSA/AppShell theme package.
- `combined` â€” website/page plus AppShell theme package, including safe live theme settings and token profile when needed.
...
10. `/audit /combined`.
11. Choose an explicit dynamic target after visual approval, such as `/usesitegraph /for /bricksbindings` or `/usebrickscontext /for /dynamictags`.
12. `/convert /bricks`.
...
That conversion can additionally emit `kiwe.sitegraph-post-import-verification.v1`, which preserves binding-to-element proof and recommends—but never activates—Bricks template conditions. This keeps target-specific route IDs, existing header/footer ownership, and global archive/single scope behind explicit review.
...
After the dynamic binding pass is accepted, use `/convert /bricks` to create the native Bricks My Templates upload JSON:
...
For vendor-neutral browser AI, IDE and tool access, first open `/wp-json/dsa/v1/ai/client-manifest` and use `/wp-json/dsa/v1/ai/client-adapters`. Ordinary external work must import `/wp-json/dsa/v1/ai/openapi.task.json`, not the full authority-discovery schema. Prefer the short-lived connection package downloaded from `Kiwe > SiteGraph`; its `kiwe_task_...` capsule is limited to public SiteGraph data plus read/convert/validate operations and cannot enter staging or mutation routes. Its `/design-context` route and MCP `kiwe_sitegraph_design_context` tool return the same safe design-evidence contract as the file export. Claude, Cursor and other MCP clients can run `mcp/sitegraph-client.js` with `KIWE_SITEGRAPH_BASE_URL` and `KIWE_SITEGRAPH_TASK_TOKEN`; the adapter rejects permanent keys, remote plaintext HTTP, oversized responses and redirects. The `/wp-json/dsa/v1/ai` URL is only a namespace. A client must use a discovered route and configure the bearer secret outside model conversation context. Permanent `kiwe_ai_...` keys remain available for explicitly authorized staging chains and must use the smallest scopes possible.
...
On WordPress 7+ / MCP Adapter capable sites, the same early connector path is available as WordPress abilities: `dsa/get-site-graph`, `dsa/get-sitegraph-design-context`, `dsa/validate-bindings`, `dsa/prepare-apply-plan`, and `dsa/stage-apply-plan`. Treat these as safe connector surfaces: only `dsa/stage-apply-plan` writes, and it writes Kiwe internal staging metadata rather than Bricks, WordPress page content, WooCommerce, or publish state.
...
Standalone Framework profiles import/export under `Kiwe > Framework`. Theme packages install under `Kiwe > Theme > Installed themes`. AI staging clients can apply a Framework profile with `kiwe.framework-profile.apply` and then explicitly push the active profile to Bricks with `kiwe.framework.push-bricks`. A Framework profile must be self-contained for the Bricks foundation: `settings.tokens.bricks_theme_style.enabled` is `true`, with a safe `id`, human `label`, and only safe global style slots. Kiwe normalizes those slots into universal tokens and writes the native Bricks Theme Style during push. `/audit /frameworkprofile` must also verify core live token coverage without human spoon-feeding: `color-brand`, `color-accent`, `color-surface`, `color-surface-raised`, `color-text`, `color-text-muted`, `color-border`, `font-display`, `font-body`, `type-h1`, `type-body`, `space-md`, `radius-lg`, and `shadow-md` must be covered directly in `settings.tokens.overrides` or through mapped `bricks_theme_style` slots. Use official variables such as `--kiwe-color-brand` and `--kiwe-space-md`; do not invent `--kiwe-color-primary` or `--seam-color-primary`.
