...
|---|---:|---:|---|
| **Current WordPress implementation** | **97%** | **3%** | The Surface, trust, commerce, PhoneKey hardening, PWA, notifications, analytics, Bricks, registry snapshot, native read-only islands, Search, enriched Saved, canonical tokens, responsive geometry, controlled offline editorial, and APEX acceptance contracts exist. Remaining implementation is concentrated in runtime cleanup, admin trigger/IA work, schema cache refinement, account saved-state migration, live proof, and release operations. |
...
| Ponytail: delete S18 Asset Build Service | **Adopted in 0.6.32** | The generated stylesheet pilot was retired after proving it added admin/runtime complexity without a production benefit beyond packaged CSS. Packaged stylesheet authority is now the only live delivery path; old generated uploads are inert and not registered by Kiwe. |
...
| S18 | L | High | **Retired in 0.6.32.** The generated stylesheet pilot is removed from runtime/admin and is no longer a release gate. Kiwe keeps packaged stylesheet authority until a future delivery experiment proves a real production benefit under a new contract. |
...
SEO and advertising safety are explicit 1.0 gates. Kiwe preserves server-rendered WordPress documents, canonical/meta/schema authority, ordinary full navigation, and no-JavaScript reachability. The audit must additionally prove crawler parity, no cloaking, ad-script survival, consent compatibility, layout stability, cache/CDN behavior, and non-intrusive initial defaults. S15 and S17 remain off-by-default pilots unless separately certified.
...
- The main `Kiwe` overview owns build/readiness status.
...
SeamFlow now has a plugin-hosted deterministic REST lane in addition to the raw Git/Node toolkit and future MCP tooling. Browser AIs that receive `KIWE_REST_BASE` plus a Kiwe AI key can call `/wp-json/dsa/v1/ai/seamflow/*` for artifact classification, Seam rebuild, Framework profile generation, Bricks conversion handoff, accessibility plan/audit, and step/full-flow orchestration. This route is designed as command-central proof, not creative authorship: it returns compact `PASS`/`FAIL`/`WARN`, files, validator proof, and next commands, and it refuses manual-only closure when a validator or native Bricks converter is unavailable. The `seamflow` key scope is narrow, while `studio_ai`, `bricks_ai`, and `all` may satisfy it for broader AI workflows.
