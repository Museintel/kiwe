...
assert.match(pinned || "", /^7\.0\.0-rc\d+$/, "PhoneKey must pin an audited Baileys 7 release candidate exactly.");
...
assert.match(transport, /fetchLatestWaWebVersion/, "PhoneKey must negotiate the current WhatsApp Web protocol version.");

if (process.env.PHONEKEY_SKIP_UPSTREAM_VERSION_CHECK !== "1") {
  ...
  assert.equal(upstream, pinned, `Baileys latest moved to ${upstream}; review compatibility before updating PhoneKey.`);
}

console.log(`PASS PhoneKey Baileys compatibility gate (${pinned})`);
