...
const baseUrl = String(process.env.KIWE_KEY_BASE_URL || "").replace(/\/$/, "");
assert.match(baseUrl, /^https:\/\//, "KIWE_KEY_BASE_URL must be an HTTPS URL.");
...
function signed(raw, options = {}) {
  ...
  return {
    ...
    "x-kiwe-key-key-id": keyId,
    "x-kiwe-key-timestamp": timestamp,
    "x-kiwe-key-nonce": requestNonce,
    "x-kiwe-key-signature": options.invalidSignature ? "0".repeat(64) : signature(tenant.secret, timestamp, requestNonce, raw),
  };
}
...
for (const path of ["/runtime-config.json", "/.kiwe-key-state/creds.json", "/src/server.mjs", "/package.json"]) {
...
console.log(`Live Key.kiwe smoke passed (${results.length}/${results.length}). No WhatsApp message was sent.`);
