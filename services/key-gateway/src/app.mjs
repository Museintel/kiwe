...
function message(site, code) {
  return `${site}: Your Key.kiwe verification code is ${code}. It expires in 10 minutes. Never share this code.`;
}
...
export function createApp(config, transport, clock = () => Date.now(), history = null, operator = {}) {
  ...
  function authenticate(request, raw) {
    const keyId = String(request.headers["x-kiwe-key-id"] || request.headers["x-phonekey-key-id"] || "");
    const timestamp = String(request.headers["x-kiwe-key-timestamp"] || request.headers["x-phonekey-timestamp"] || "");
    const nonce = String(request.headers["x-kiwe-key-nonce"] || request.headers["x-phonekey-nonce"] || "");
    const supplied = String(request.headers["x-kiwe-key-signature"] || request.headers["x-phonekey-signature"] || "");
  ...
  return async function app(request, response) {
    const url = new URL(request.url || "/", config.publicBaseUrl || "https://key.kiwelaunch.com");
    if (operator.web) {
      await operator.web.handle(request, response);
    ...
    if (request.method === "GET" && url.pathname === "/setup") {
      ...
      const compatibilityNotice = setup.registered === false
        ? `<aside><strong>New-device compatibility notice</strong><br>WhatsApp may currently reject unofficial linked-device sessions after a valid scan. If the phone says it could not link, stop retrying. Key.kiwe keeps email fallback active while WhatsApp is unavailable.</aside>`
        : "";
      ...
      const setupBody = image
        ...
        : `<p>${setup.state === "open" ? "The WhatsApp sender is connected." : (setup.state === "pairing-timeout" ? "The pairing window ended without a completed WhatsApp handshake. Key.kiwe will prepare a fresh session after a guarded cooldown." : "Reload shortly while Key.kiwe prepares the pairing session.")}</p>`;
      const html = `<!doctype html><html><head><meta name="viewport" content="width=device-width"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; img-src data:; style-src 'unsafe-inline'; base-uri 'none'; form-action 'self'"><meta http-equiv="refresh" content="15"><title>Key.kiwe WhatsApp setup</title><style>body{font:16px system-ui;max-width:620px;margin:8vh auto;padding:24px;background:#f5f7f4;color:#10251d}main{background:#fff;border:1px solid #ccd8d1;border-radius:18px;padding:28px}img{display:block;max-width:100%;margin:20px auto}code{padding:4px 7px;background:#edf3ef;border-radius:5px}.metrics{display:flex;gap:10px;flex-wrap:wrap;color:#52645d;font-size:13px}.diagnostic{font-size:13px;color:#52645d}aside{margin:18px 0;padding:14px 16px;border:1px solid #d5a72e;border-radius:12px;background:#fff8df;color:#5b4300;line-height:1.45}form{margin-top:14px}button{appearance:none;border:0;border-radius:9px;background:#10251d;color:#fff;font:inherit;font-weight:700;padding:10px 14px;cursor:pointer}a{color:#075b3a}</style></head><body><main><h1>Key.kiwe WhatsApp</h1><p>State: <code>${escapeHtml(setup.state)}</code></p><p class="metrics"><span>Transport: ${escapeHtml(transport.name)}</span>${library}${protocol}<span>Memory: ${rssMb} MB / ${config.memoryLimitMb} MB</span><span>Uptime: ${Math.floor(process.uptime() / 60)} min</span></p>${compatibilityNotice}${resetAction}${lastDisconnect}${setupBody}${historyLink}</main></body></html>`;
    ...
    if (request.method === "POST" && url.pathname === "/setup/reset-session") {
      ...
      return text(response, 202, "<!doctype html><html><head><meta name=\"viewport\" content=\"width=device-width\"><meta http-equiv=\"refresh\" content=\"4;url=/setup?token=" + encodeURIComponent(config.setupToken) + "\"><title>Key.kiwe reset</title></head><body><main><h1>Linked device reset</h1><p>Key.kiwe is restarting. A fresh pairing QR will appear shortly.</p></main></body></html>", "text/html; charset=utf-8");
    }
    if (request.method === "GET" && url.pathname === "/rc") {
      ...
      const html = `<!doctype html><html><head><meta name="viewport" content="width=device-width"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; style-src 'unsafe-inline'; base-uri 'none'; form-action 'none'"><meta http-equiv="refresh" content="20"><title>Key.kiwe RC history</title><style>body{font:14px system-ui;margin:24px;background:#f4f7f5;color:#10251d}main{background:#fff;border:1px solid #ccd8d1;border-radius:16px;padding:22px;overflow:auto}table{border-collapse:collapse;width:100%;min-width:920px}th,td{text-align:left;padding:9px;border-bottom:1px solid #e0e8e3;vertical-align:top}th{position:sticky;top:0;background:#edf4ef}.note{color:#52645d}</style></head><body><main><h1>Key.kiwe RC delivery history</h1><p class="note">Capped at ${config.rcObservability.maxEvents} events for ${config.rcObservability.retentionDays} days. OTP codes are never stored. Inbound text capture: ${config.rcObservability.captureInboundText ? "enabled" : "disabled"}. Consented outbound notification capture: ${config.rcObservability.captureOutboundText ? "enabled" : "disabled"}.</p><table><thead><tr><th>Time</th><th>Tenant</th><th>Direction</th><th>Status</th><th>Target</th><th>Summary</th><th>Error</th></tr></thead><tbody>${rows || '<tr><td colspan="7">No events yet.</td></tr>'}</tbody></table></main></body></html>`;
    ...
    try {
      ...
      const controlSite = operator.controlPlane?.siteByKeyId(keyId);
      if (controlSite && !controlSite.active) return json(response, 403, { ok: false, error: "site_inactive" });
      operator.controlPlane?.recordTransaction(keyId, url.pathname.replace(/^\/v1\//, ""), now);
      ...
      const tenantTransport = await operator.transportForTenant?.(keyId) || transport;
      if (url.pathname === "/v1/message") {
        ...
        if (!(await tenantTransport.ready())) {
        ...
        try {
          const sent = await tenantTransport.sendText(phone, messageBody);
      ...
      await record({ tenant: keyId, phone, requestId, direction: "outbound", status: "requested", summary: "Key.kiwe verification requested" });
      if (idempotency.has(`${keyId}:${requestId}`, now)) return json(response, 200, { ok: true, duplicate: true, provider: "whatsapp" });
      ...
      const sent = await tenantTransport.sendText(phone, message(site || tenant.label, code));
      idempotency.add(`${keyId}:${requestId}`, 15 * 60 * 1000, now);
      await record({ tenant: keyId, phone, requestId, receipt: sent.id, direction: "outbound", status: "accepted", summary: "WhatsApp accepted the Key.kiwe verification message" });
      return json(response, 202, { ok: true, provider: "whatsapp", receipt: opaque(sent.id || requestId) });
    } catch (error) {
