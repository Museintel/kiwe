...
Use `Kiwe > SiteGraph > Download client connection` for authenticated OpenAPI tools, Claude/Cursor MCP clients, IDEs and the maintained SEAM Compiler extension. The downloaded `kiwe.external-client-connection.v1` document contains a short-lived task capsule plus a `kiwe.external-client-adapters.v1` descriptor. The capsule is public-data-only and limited to SiteGraph, Bricks context, SEAM conversion and validators. It has expiry, row, field, resource and request budgets; Kiwe stores only its hash. It never grants staging, publishing, runtime or mutation access. Configure the secret in the client's secure tool/action layer and delete the downloaded connection file afterward. The maintained local MCP entrypoint is `kiwe-ai-toolkit/mcp/sitegraph-client.js`; it accepts only `kiwe_task_*`, HTTPS target URLs and non-redirecting JSON responses.
...
Use the workflow file when the human wants high-quality output, fewer correction loops, or command-style phases such as `/list`, `/redo`, `/fix`, `/document`, `/ideate`, `/rebuild /seamframework`, `/audit /seamframework`, `/create /frameworkprofile`, `/audit /frameworkprofile`, `/create /brickstheme`, `/audit /brickstheme`, `/create /dsatheme`, `/create /preview /dsatheme`, `/assemble /combined`, `/create /preview /combined`, `/usesitegraph /for /bricksbindings`, `/usebrickscontext /for /dynamictags`, `/convert /bricks`, `/audit /bricksconversion`, `/create /accessibility`, `/audit /accessibility`, or `/fix /accessibility`.

For every later slash command in an existing AI conversation, fetch `https://start.kiwelaunch.com/.well-known/kiwe.json?refresh={new-nonce}`, verify its `contractVersion`, `releaseId`, and `sourceHash`, and use only the selected content-hash URL in `immutableCommands` plus its pinned resource hashes. Canonical registry responses are no-store; content-hash release responses are immutable. `/redo` means the immediate previous output failed: discard it and rerun the same output-producing instruction from its approved input snapshot under the freshly discovered release. Do not require the human to explain the failure. It is not a synonym for localized `/fix`.
...
```text
Kiwe > SiteGraph > SiteGraph workspace > Download SiteGraph JSON
```
...
Kiwe Companion AI is the optional site-aware context broker for external AIs creating website/page, DSA/AppShell theme, combined, dynamic binding, audit, staging, or security-support outputs. Enable it in `Kiwe > AI`, then issue a revocable key with `companion` scope. The Companion returns compact context cards, route hints, validation diffs, rule IDs, and safe next-action plans rather than dumping the whole plugin or raw security logs. For revisions, browser AI should submit the actual file map to `/ai/audit-companion/review` first, fix every `mustFix` item, then run its own explanation/self-audit. This saves tokens because the deterministic Audit Companion identifies concrete contract failures without calling a model. Its local memory stores privacy-safe fingerprints, structured pass/fail finding codes, and counts only, never secrets, raw visitor trails, raw SecureTrack events, customer data, full handoff files, or unredacted transcripts. SecureTrack AI exposure is a toggle under `Kiwe > AI`: redacted SecureTrack briefs use Companion consent/scopes, while optional Site Brain cloud review sends only hashed/redacted packets through the isolated `securetrack` profile of the shared Kiwe AI broker. SecureTrack has no separate provider key, direct provider request path, or shared conversation memory. `Kiwe > Secure` remains focused on human security controls and enforcement.

Kiwe AI provider configuration is site-wide and singular. On WordPress 7, choose `WordPress AI Client` in Kiwe and configure the actual provider once under `Settings > Connectors`; Kiwe does not read that Connector credential. Older sites may use Kiwe's encrypted provider fallback under `Kiwe > AI`. Studio, SiteGraph, SecureTrack, and Bricks are separate broker consumers with distinct capabilities, data boundaries, prompt/rate budgets, and output validators. The broker is stateless and rejects conversation-history fields, so using one provider cannot mix one service's context into another. Approved plugins may register a least-privilege profile through `kiwe_ai_broker_profiles`, receive the broker from `kiwe_ai_broker_ready`, or access `Plugin::instance()->ai_broker()`; none of these surfaces exposes the provider credential.
...
For `/convert /bricks`, run the deterministic SEAM Compiler and produce native Bricks My Templates upload JSON by default. Raw Convert does not require a Framework Profile and must not inject Seam Framework. Use `/seamframework` only when the human explicitly wants the optional profile-backed optimization stage.
...
The conversion must preserve the approved page hierarchy, classes/attributes, canonical `data-dsa-open-module` launchers, query-loop intent, dynamic tags, conditions, interactions, and unsupported/manual-review evidence. The goal is an editable Bricks visual-builder handoff, not a page that only renders through one CSS dump: ordinary typography, colors/backgrounds/gradients, borders/radii, shadows, transforms, filters, transitions, spacing, sizing, grid/flex, responsive direction, alignment, query loops, conditions, interactions, and dynamic tags should become Bricks-native controls first. In raw mode, source literals and authored classes are allowed when required for 1:1 fidelity. In `/seamframework` mode, Theme Style, universal tokens, project variables/classes, element exceptions, and scoped CSS form the strict ownership ladder; Framework-dependent template settings consume defined variables without fallbacks. Validate with the SEAM Compiler proof engine before staging. The package does not mutate WordPress or Bricks by itself.

`/convert /bricks` discovers arbitrary website pages and may split a complete document into dedicated header, content, and footer templates. Never convert a separately identified `combined-preview`, `appshell-theme`, DSA/AppShell screen/sheet/dock/navbar lane, `theme-package.json`, or `css/theme.css` into Bricks page content.
...
For staging proof after uploading the MU folder, use the latest `wp-content/mu-plugins/dsa/site-graph-system/release-proof-*.md` file. Version `6.78` records the SeamFlow identity for the full-flow `KIWE-START.md` / `entry.json` front door; external-AI vs Kiwe Internal AI/Companion boundaries; browser AI, IDE AI, MCP, and skill-capable workflows; page/header/footer/template Bricks target guidance; content-based attachment diagnostics; Start-URL-only `/list` orientation; approval-gated audit/fix/create/convert phases when no command is supplied; artifact classification; explicit `/execute /stepbystep` and `/execute /fullflow` commands; audit-closure law requiring `/audit` -> `/fix` -> same `/audit` loops until PASS from any detected start point; current-run-only evidence boundaries that reject stale National Chikki/BioVantage/prior-output material unless supplied in the current turn; `/usecompanion` handshake and fallback behavior; `kiwe_seamflow_plan` plus compatibility alias `kiwe_plan_flow`; compact command manifest routing; no-clone command routing; `/fix /accessibility` dark-mode token remap recipe; public Bricks 2.3.x native-template compatibility; DSA runtime dark-state sync across `data-kiwe-theme` and `data-brx-theme`; `/fix /accessibility` structural-preservation contract; canonical `/convert /bricks` command; native Bricks upload JSON validation; standalone-file audit support; Framework-profile Bricks Theme Style checks; Studio AI operating-mode routes; native-provider boundary; Bricks AI intelligence routes; Bricks editor companion toggle; SecureTrack shared AI settings boundary; API proof routes; WordPress 7 ability checks; dynamic handoff checks; browser smoke checks; mutation boundaries; deterministic `/create /accessibility` + `/audit /accessibility` color/dark-mode/text-containment lane; tightened Bricks template-upload validator checks; fail-level `/convert /bricks` token-purity checks for direct component colors in native Bricks settings/global classes/custom CSS; and fail-level Bricks variable-availability checks requiring `var(--token, fallback)` in native settings/global classes unless the workflow explicitly gates import behind a verified Kiwe > Framework push.
