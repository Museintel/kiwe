...
For every later slash command in an existing AI conversation, fetch `https://start.kiwelaunch.com/.well-known/kiwe.json?refresh={new-nonce}`, verify its `contractVersion`, `releaseId`, and `sourceHash`, and use only the selected content-hash URL in `immutableCommands` plus its pinned resource hashes. Canonical registry responses are no-store; content-hash release responses are immutable. `/redo` discards the immediate previous command candidate and reruns that same command from its approved input snapshot under the freshly discovered release. It is not a synonym for `/fix`.
