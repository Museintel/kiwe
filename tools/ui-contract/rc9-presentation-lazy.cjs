...
( async function () {
	...
	check( 'Persistent shell is below the RC9B raw budget', Buffer.byteLength( core ) < 350000, `${ Buffer.byteLength( core ) } bytes` );
	...
	check( 'Surface control font stacks use Kiwe font token', !/font:\s*[0-9]+[^;]*Inter,ui-sans-serif/.test( css ) && css.includes( 'font: 800 18px/1.2 var(--kiwe-font-ui);' ) && css.includes( 'font: 900 13px/1 var(--kiwe-font-ui);' ) );
	...
	check( 'Dismissible AI popouts share swipe behavior', core.includes( 'const popoutDismissible = isAiNotificationDismissible( insight );' ) && core.includes( 'bindAiNotificationSwipe( aiPopout )' ) && css.includes( '.dsa-ai-popout[data-dsa-ai-dismissible]' ) && /\.dsa-ai-insight\.is-swiping,\s*[\s\S]*?\.dsa-ai-popout\.is-swiping\s*\{[\s\S]*?transition:\s*none;/.test( css ) );
