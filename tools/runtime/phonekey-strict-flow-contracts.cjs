...
check('new-device recovery asserts an existing passkey before offering enrollment',
  core.includes('function pk_new_device_recovery_response')
  && core.includes("$has_passkey ? 'login_passkey' : 'enroll_passkey'")
  && core.includes("$has_passkey ? 'device_passkey_assert' : 'device_passkey_enroll'")
  && surface.includes('Use existing passkey')
  && surface.includes('Add passkey to this device')
);
check('duplicate synced credentials return to assertion instead of exposing a browser error',
  surface.includes("error.name === 'InvalidStateError'")
  && surface.includes('That passkey is already connected to this account.')
  && /passkeyDuplicateCredentialError[\s\S]*?renderPasskeyLogin/.test(surface)
);
check('unsupported in-app browsers receive a precise full-browser recovery path',
  surface.includes('window.PublicKeyCredential')
  && surface.includes('passkeyInAppBrowser')
  && surface.includes('window.isSecureContext')
  && surface.includes('Open in full browser')
  && surface.includes('browser isolation means the verification step must be repeated')
);
...
check('native password authority is rendered as an isolated site-branded Key surface',
  core.includes('function pk_brand_wordpress_login')
  && core.includes("add_action( 'login_enqueue_scripts'")
  && core.includes("add_filter( 'login_headerurl'")
  && core.includes('Site_Identity_Service::logo_url()')
  && core.includes('Private, ad-free access by Key.kiwe')
);
...
check('strict PhoneKey exposes the native password setup and reset lane inside Key branding', surface.includes('Set or reset secure password') && surface.includes('phonekey.resetPasswordUrl') && surface.includes('WordPress securely validates the password underneath'));
