...
check('ideate discovery artifact has a strict v5 schema, source authority and creative boundary', ideateDiscoverySchema.additionalProperties === false && ideateDiscoverySchema.properties.schema.const === 'kiwe.ideate-discovery.v5' && ['designContextCoverage', 'approval', 'sourceAuthority', 'contentReadiness'].every((key) => ideateDiscoverySchema.required.includes(key)) && ideateDiscoverySchema.properties.sourceAuthority.properties.status.enum.includes('approved-final') && ideateDiscoverySchema.properties.nextAction.enum.includes('ask-source-authority') && ideateDiscoverySchema.properties.creativeBoundary.properties.frameworkNeutral.const === true);
const productionGate = ideateDiscoverySchema.properties.contentReadiness.allOf.find((rule) => rule.if.properties.status.const === 'production-ready')?.then.properties;
check('production readiness excludes placeholders and unproven claims and requires runtime evidence', productionGate?.placeholderRegions.maxItems === 0 && productionGate?.unresolvedClaims.maxItems === 0 && productionGate?.runtimeSmokeTest.const === 'passed' && JSON.stringify(productionGate?.basis.enum) === JSON.stringify(['sitegraph-grounded', 'self-contained-approved-source']));
