...
check('resource selection stores attachment references and never deletes media', profile.includes("'attachment' !== get_post_type") && profile.includes("'resources' => [ 'items' => $resource_items ]") && onboarding.includes('never deletes the attachment or file') && !onboarding.includes('wp_delete_attachment'));
