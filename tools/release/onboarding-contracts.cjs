...
check('owner journey is segmented and media-picker enabled', ['Identity','Story','Contact','Brand','Website plan','Store','Resources','Review'].every((x) => onboarding.includes(x)) && onboarding.includes('wp_enqueue_media') && js.includes('wp.media'));
...
check('WooCommerce remains price shipping-zone and product authority', profile.includes("update_option( 'woocommerce_currency'") && onboarding.includes('does not invent tax rates or overwrite shipping zones') && !profile.includes('WC_Shipping_Zone') && !profile.includes("'post_type'=>'product'"));
check('page intent does not create pages', profile.includes('PAGE_VISIBILITY_META') && onboarding.includes('Kiwe does not create the pages') && !profile.includes("'post_type'=>'page'"));
...
check('founder and team context link to WordPress users without changing account authority', onboarding.includes('Link to an existing WordPress user') && onboarding.includes('No login, email, capability or WordPress role is changed or exposed') && profile.includes('apply_people_context') && profile.includes("user_can( $actor_id, 'edit_user'") && !profile.includes("'user_email'=>") && !profile.includes("'role'=>"));
check('optional team repeater preserves pending profiles and stable member selectors', onboarding.includes('data-kiwe-team-toggle') && onboarding.includes('data-kiwe-team-member-template') && js.includes('data-kiwe-add-team-member') && profile.includes("'member-' . substr( wp_generate_uuid4()") && profile.includes("'team' => [ 'enabled'"));
check('Bricks uses native user loops plus Kiwe-only team fields', graph.includes("'objectType'=>'user'") && graph.includes("'engine'=>'WP_User_Query'") && ['{wp_user_display_name}','{wp_user_description}','{wp_user_picture}'].every((tag) => graph.includes(tag)) && ['kiwe_team_title','kiwe_team_image','kiwe_team_linkedin_url'].every((tag) => bricks.includes(tag)));
check('founder LinkedIn and selector-addressable team fields reach Bricks SiteGraph and WP7', bricks.includes('kiwe_founder_linkedin_url') && graph.includes("'syntax'=>'{kiwe_team_FIELD:MEMBER_ID}'") && bindings.includes("'kiwe/team'") && bindings.includes("'selectorArgument' => 'memberId'"));
check('services bind to developer-owned public custom post types without registering a duplicate type', profile.includes('service_post_types') && profile.includes("get_post_types( [ 'show_ui'=>true ]") && profile.includes('apply_services_context') && enhancement.includes('ownerContext.services') && !profile.includes('register_post_type'));
check('unbound services remain pending while bound services use native post fields', onboarding.includes('Not configured yet · keep as a plan') && onboarding.includes('Native service #') && profile.includes("'post_title'=>$item['title']") && profile.includes("'post_excerpt'=>$item['summary']") && profile.includes('set_post_thumbnail'));
check('service taxonomy paths support hierarchical client language and native term assignment', onboarding.includes('Parent > Child') && profile.includes('ensure_term_paths') && profile.includes('wp_insert_term') && profile.includes('wp_set_object_terms'));
check('developer-registered REST-visible service fields remain native post meta', profile.includes('service_meta_fields') && profile.includes("get_registered_meta_keys( 'post', $post_type )") && profile.includes("empty( $args['show_in_rest'] )") && profile.includes('update_post_meta') && onboarding.includes("$service['meta']"));
check('service writes honor native post term and meta capabilities', profile.includes("$object->cap->create_posts") && profile.includes("'edit_post_meta'") && profile.includes("$object->cap->assign_terms") && profile.includes("$object->cap->manage_terms"));
