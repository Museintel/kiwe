...
check('test workspace restore excludes identities orders credentials and media binaries', snapshot.includes("'users'             => 'untouched'") && snapshot.includes("'orders'            => 'untouched'") && snapshot.includes("'credentials'       => 'untouched'") && snapshot.includes("'mediaBinaries'     => 'untouched'") && snapshot.includes("$excluded = [ 'attachment', 'shop_order', 'shop_order_refund'") && !/\$types\s*=\s*\[[^\]]*'shop_order'/.test(snapshot));
