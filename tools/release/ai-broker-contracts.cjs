...
check('Gemini JSON consumers request structured output with bounded thinking', broker.includes("'responseMimeType'") && broker.includes("'thinkingLevel'") && provider.includes("$config['responseMimeType'] = 'application/json'") && provider.includes("$config['thinkingConfig']") && provider.includes("! empty( $part['thought'] )"));
