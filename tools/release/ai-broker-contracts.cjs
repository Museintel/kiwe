...
check('Gemini JSON consumers request schema-guided structured output with bounded thinking', broker.includes("'responseMimeType'") && broker.includes("'thinkingLevel'") && broker.includes("response_schema( $request['responseSchema']") && provider.includes("$config['responseMimeType'] = 'application/json'") && provider.includes("$config['responseJsonSchema']") && provider.includes("'thinking_level'") && provider.includes("! empty( $part['thought'] )"));
check('Gemini 3 uses stateless Interactions with header credentials and preserves provider failures', provider.includes("'https://generativelanguage.googleapis.com/v1/interactions'") && provider.includes("$headers['x-goog-api-key'] = $key") && provider.includes("'store' => false") && provider.includes("'response_format'") && provider.includes("'model_output'") && broker.includes("! empty( $result['ok'] ) && empty( $validation['valid'] )"));
check('provider capacity retry is bounded and excludes quota and authentication failures', provider.includes('while ( $attempts < 2 )') && provider.includes("[ 500, 502, 503, 504 ]") && provider.includes("'attempts' => $attempts") && !provider.includes('[ 429, 500, 502, 503, 504 ]'));
